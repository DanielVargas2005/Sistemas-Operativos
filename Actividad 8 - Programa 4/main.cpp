/*
 Aprendizaje 8 programa 4 - Algoritmo de planificación FCFS (First Come First Server)
...
public:
    ...
    unsigned int getFirstExecutionTime() const {
...
void simulateProcessing(vector<Process>& newProcesses, int numberProcesses, vector<unsigned int>& usedIds);
...
int main() {
    ...
    cin.ignore();
    simulateProcessing(newProcesses, numProcesses, usedIds);
    return 0;
}
...
void simulateProcessing(vector<Process>& newProcesses, int numberProcesses, vector<unsigned int>& usedIds) {
    ...
    while (completedProcessesForBcp.size() < totalProcessCount) {
        ...
        if (executingProcess != nullptr && executingProcess->getTimeElapsed() >= executingProcess->getMaxTime()) {
            ...
            finishedProcessesForDisplay.push_back({(int)executingProcess->getId(), executingProcess->getCalculator().getOperationString(), ss.str()});
    ...
    auto printRow = [](const Process& p, const string& state, const string& result, const string& blockedInfo) {
        ...
        if (state == "Terminado") {
            if (p.hasErrorTerminated()) {
                ...
                op_res_str = p.getCalculator().getOperationString() + "=" + result;
            }
        } else if (state == "Nuevo") {
            op_res_str = "-";
        } else {
        ...
        if(state == "Listo" || state == "Ejecucion" || state == "Bloqueado"){
            ...
            unsigned int serviceT = p.getTimeElapsed();
            if (!p.hasErrorTerminated() && p.getTimeElapsed() >= p.getMaxTime()) {
                serviceT = p.getMaxTime();
            }
            unsigned int returnT = p.getCompletionTime() - p.getArrivalTime();
            currentWait = returnT - serviceT;
        }
        cout << setw(12) << (state != "Nuevo" ? to_string(currentWait) : "-");

        if (p.hasExecutedFirstTime()) {
            unsigned int liveResponseTime = p.getFirstExecutionTime() - p.getArrivalTime();
            cout << setw(12) << liveResponseTime;
        } else {
            cout << setw(12) << "-";
        }
        ...
        string returnStr = "-";
        if (state == "Terminado") {
            returnStr = to_string(p.getCompletionTime() - p.getArrivalTime());
        }
        cout << setw(12) << returnStr;

        cout << setw(12) << (p.getTimeElapsed() > 0 || state == "Terminado" ? to_string(p.getTimeElapsed()) : "0");
        cout << setw(15) << (state != "Terminado" ? to_string(p.getMaxTime() - p.getTimeElapsed()) : "0");
        cout << setw(8) << p.getMaxTime();

        cout << endl;
    };

    for (const auto& p : newProcesses) printRow(p, "Nuevo", "-", "");
    for (const auto& p : readyProcesses) printRow(p, "Listo", "-", "");
    if (executingProcess) printRow(*executingProcess, "Ejecucion", "-", "");
    for (const auto& pair : blockedProcesses) {
        string blockedInfo = to_string(BLOCKED_TIME - (globalCounter - pair.second));
        printRow(pair.first, "Bloqueado", "-", blockedInfo);
    }
    for (const auto& p : completedProcesses) {
         stringstream ss;
         ss << fixed << setprecision(2) << p.getCalculator().calculate();
         printRow(p, "Terminado", ss.str(), "");
    }
