    ...
    for (int i = C_0; i < processMatrix.size(); i++) {
        ...
        for (int j = C_0; j < PROCESS_NUMBER; j++) {
            ...
            if (programmerName.length() > MAX_NAME_SIZE) {
                cout << "Error: Nombre demasiado largo (max " << MAX_NAME_SIZE << " caracteres)\n";
                continue;
            }

            bool nombreValido = true;
            for (char c : programmerName) {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (temp <= 0) {
                        throw out_of_range("El tiempo debe ser mayor a 0");
                    }
            ...
            while (true) {
                ...
                try {
                    ...
                    if (temp <= 0) {
                        throw out_of_range("El ID debe ser mayor a 0");
                    }
                    if (!isIdUnique(processMatrix, temp, count)) {
                        throw runtime_error("El ID ya existe. Debe ser unico.");
                    }
            ...
            while (true) {
                ...
                try {
                    ...
                    if (input.length() != 1) {
                        throw invalid_argument("Ingrese exactamente un caracter");
                    }
            ...
            while (true) {
                ...
                try {
                    ...
                    if ((operation == '/' || operation == '%') && operandB == 0) {
...
void displayStatus(const vector<Process>& loteActual,
    ...
    for (int i = 0; i < maxAltura; i++) {
        ...
        if (i < terminados.size()) {
            ...
            streamA << fixed << setprecision(2) << calc.getOperandA();
            streamB << fixed << setprecision(2) << calc.getOperandB();
            string operacion = streamA.str() + opStr + streamB.str();
            ...
            cout << setw(20) << terminadoInfo << left;
        } else {
            cout << setw(20) << "";
        }
        cout << endl;
