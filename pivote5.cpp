...
// Función para verificar si un ID ya existe en la matriz
bool isIdUnique(const vector<vector<Process>>& processMatrix, unsigned int id, int currentCount) {
    for (int i = C_0; i < processMatrix.size(); i++) {
        for (int j = C_0; j < PROCESS_NUMBER; j++) {
            // Solo verificar hasta los procesos que ya hemos llenado
...
// Función para llenar la matriz de procesos (CON VALIDACIÓN DE ID ÚNICO)
void fillProcessMatrix(vector<vector<Process>>& processMatrix, int numberProcesses) {
    string programmerName;
    char operation;
...
// Función para mostrar el estado actualizado
void displayStatus(const vector<Process>& loteActual,
                  const Process* procesoEjecutando,
                  int tiempoTranscurrido,
                  int tiempoRestante,
    ...
    for (int i = 0; i < maxAltura; i++) {
        // Columna 1: Procesos del lote actual (sin el en ejecución)
        ...
        // Columna 3: Todos los procesos terminados
...
// Función para simular el procesamiento de TODOS los lotes CORREGIDA
void simulateProcessing(const vector<vector<Process>>& processMatrix, int numberProcesses) {
    ...
    for (int loteIndex = C_0; loteIndex < processMatrix.size(); loteIndex++) {
        // Obtener lote actual y filtrar procesos no nulos
        ...
        // Procesar cada proceso del lote actual
        // USAR while en lugar de for para manejar mejor la eliminación
        int i = C_0;
        while (i < loteActual.size()) {
            ...
            // ⭐ CREAR una copia temporal del lote SIN el proceso actual para mostrar
            ...
            // ELIMINAR el proceso del lote actual
            loteActual.erase(loteActual.begin() + i);
            // NO incrementar i porque acabamos de eliminar un elemento

            // Mostrar estado después de terminar (con el proceso ya eliminado)
